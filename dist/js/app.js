...
function checkStorefrontAuth() {
  ...
  if (user) {
    ...
    if (roleBadge) {
      roleBadge.textContent = user.role === 'farmer' ? `🌾 ${user.crop || 'Crop'} Farmer` : `🛡️ ${user.role.toUpperCase()} Staff`;
    }
...
window.submitStorefrontLogin = async function(e) {
  ...
  if (!identifier || !password) {
    alert('Please enter your mobile number or email and password.');
    return;
  }
  ...
  try {
    ...
    checkStorefrontAuth();
    fetchLiveProducts(); // Reload products with user-targeted sorting!
    closeModal('authModal');
    alert(`Welcome to Sathya Bio, ${data.user.name}! Your personalized crop catalog has been loaded.`);
  } catch (err) {
    alert('Could not connect to authentication server. Please check backend connection.');
  } finally {
    if (btn) {
      btn.disabled = false;
...
window.submitStorefrontRegister = async function(e) {
  ...
  if (!name || !phone || !password) {
    alert('Please provide your name, mobile number, and a password.');
    return;
  }
  ...
  try {
    ...
    fetchLiveProducts();
    closeModal('authModal');
    alert(`Account created successfully! Welcome to Sathya Bio, ${data.user.name}!`);
  } catch (err) {
...
// Check for updates periodically or on window focus so admin changes reflect immediately
